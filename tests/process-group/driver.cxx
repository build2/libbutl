...
// Signal handling (see signal handling in build2 driver for details).
//
// NOTE: don't forget to update signal handling in build2 driver if changing
//       anything here.
//
#ifndef _WIN32
static void
terminating_signal_handler (int sig, bool unblock)
{
  ...
  if (unblock)
  {
    ...
    if (pthread_sigmask (SIG_UNBLOCK, &um, nullptr /* oldset */) != 0)
      _exit (1);
  }
  ...
  case SIGABRT:
    {
      ...
      if (sigaction (sig, &sa, nullptr /* oldact */) != 0)
        _exit (1);

      terminating_signal_handler (sig, false /* unblock */);
      break;
    }
  case SIGTSTP:
  case SIGCONT:
    ...
    case SIGABRT:
      {
        ...
        terminating_signal_handler (sig, true /* unblock */);
        assert (false);
        break;
      }
    case SIGTSTP:
...
// Wait for a process to terminate and return true if the process has
...
// Convert an abnormal termination status value to its description.
//
static string
abnormal_status_to_string (DWORD status)
{
  process_exit pe (status, process_exit::as_status);
  return pe.description ();
}

// Wait for a process to terminate and return true if the process has
// terminated abnormally with the specified status. Otherwise, issue
...
//
// Note that the argv argument doesn't contain the program path, -c, and
// immediate -S, -G options.
//
static int
...
exec_tests (const path& p)
{
  ...
  // exemplars of a group leader to reap it using different functions.
  //

  // Group/job leader doesn't start any new group/job members and exits
  // normally.
  //
  {
    strings args ({"-e", "3"});
  ...
  // get reaped with their original parents.
  //
  {
  ...
  // child and exits. The group leader reaps its child.
  //
  {
  ...
  //
  {
    ...
    // While at it, test the out-of-order SIGCONTs.
    //
    assert (kill (-pr1.group, SIGCONT) == 0);
    assert (kill (-pr2.group, SIGCONT) == 0);
  ...
  // and exits. The job leader waits for its child to exit.
  //
  {
  ...
  // that the job leader didn't reap the group members.
  //
  {
    strings args ({"{", "-s", "1000", "}",
                   "{", "-s", "1000", "}",
...
//
// -k <name>
//    Send the specified signal to itself (POSIX only).
//
// -g <name>
//    Send the specified signal to the own group (POSIX only).
//
// -S
...
//
// -G
//    Run the child process as a group leader.
//
