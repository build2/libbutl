...
// Note that the STATUS_JOB_NOT_EMPTY error code may not be defined in
// windows.h. Not to mess with ntstatus.h, let's just define it ourselves.
//
#  ifndef STATUS_JOB_NOT_EMPTY
#    define STATUS_JOB_NOT_EMPTY DWORD (0xC000050F)
#  endif
...
namespace butl
{
  ...
  // process
  //
  // Protected by the process spawn mutex. See process.hxx for the gory details.
  //
#ifndef _WIN32
      ...
      : group (0)
  {
...
#endif
    {
      ...
      // If requested, execute the child process in the new process group.
      //
      if (new_group)
      {
      ...
      for (size_t i (0);; ++i)
      {
        ...
        //
        // Also note that if we execute the child process in the new process
        // group, then we encounter the following race condition:
        //
        // - We enter posix_spawn(), which waits for the child process to call
        //   exec().
        //
        // - SIGTSTP is sent to the parent's group before the child changes
        //   its group, so that the child also receives it.
        //
        // - After the child changes the group and unblocks all the signals,
        //   before it calls exec(), it gets suspended and we get stuck in
        //   the posix_spawn() call.
        //
        // - SIGCONT is sent to the parent's group, but the child doesn't
        //   receive it, since it has changed its group, and so we stuck here
        //   forever.
        //
        // See the "posix_spawn: parent can get stuck in uninterruptible sleep
        // if child receives SIGTSTP early enough" thread
        // (https://sourceware.org/pipermail/libc-help/2022-August/006262.html)
        // for additional details on this issue. Also see cargo-nextest signal
        // handling documentation for the details on their double-spawning
        // processes approach to avoid this race condition.
        //
...
#ifndef LIBBUTL_POSIX_SPAWN_CHDIR
    {
      ...
      for (size_t i (0);; ++i)
      {
        ...
        if (handle != 0) // Parent?
        {
          ...
          if (new_group && handle != -1)
          {
            ...
            // this process exists (potentially as a defunct).
            //
            if (setpgid (handle, handle) != 0 &&
                errno != EACCES               &&
                errno != ESRCH)
      ...
      if (handle == 0)
      {
        ...
        // child has got a pending signal, broadcasted to the parent's
        ...
        // the parent right after the fork() call but before the group is
        // added to the list. In this case, at the time the parent handles
        // SIGCONT (when it is resumed at the same point, right after fork()),
        // the child's group is not in the list yet and so the child will not
        // be resumed. To resolve this, we can probably always send SIGCONT to
        // the newly created group (or just the child process) right after we
        // added it to the list. Let's, however, keep it simple for now and
  ...
  //
  static void
  wait_pending_signal (int sig, bool ignore_error = false)
  {
    while (true)
    {
      sigset_t sigmask;
    ...
    if (handle != 0)
    {
      ...
      if (group == 0) // Not a process group leader?
      {
        ...
        else
        {
          ...
          // such a signal, it's likely that the same signal will be received
          // by the parent process as well. The reason for such an assumption
          // is that these signals are normally sent for the whole process
          // group (SIGINT by the terminal on Ctrl+C, etc). Note that while
          // the signals are delivered to the group's processes
          // simultaneously, they are handled by these processes with random
          // delays in arbitrary order. This causes a race condition, when the
          // parent process may complain on the abnormally terminated children
          // before being terminated itself due to the same signal. This
          // normally happens when the parent process installs a custom signal
          // handler which performs some cleanups before terminating the
          // process. Thus, to decrease the probability of such an unwanted
          // (though perfectly valid) scenario, we will wait while the signal
          // which terminated the child process stays pending for the
          // parent. Here we assume that from the moment the signal stopped
          // being pending, the execution of the signal handler begins and
          // it's now the handler's responsibility to prevent the described
          // scenario (it, for example, may disable issuing diagnostics before
          // starting cleanups, etc).
          //
      ...
      else
      {
        ...
        {
          siginfo_t info;
          r = waitid (P_PID, handle, &info, WEXITED | WNOWAIT);
          ...
          // combination. Let's retry waitid() on unexpected statuses for
          // MacOS.
          //
#ifdef __APPLE__
          if (r == 0                     &&
        ...
        // Stash the potential error, since the mutex lock can spoil it.
        //
        int err (errno);

        // Lock the mutex to safely remove the group from the list.
        //
        // Keep it locked until the check for the unreaped group members is
        // complete, to decrease the chances of the group id reuse (see below
        // for details).
        //
        ulock l (mutex);
        ...
        if (r == -1)
        {
          // If ignore errors then give up on trying to kill the unterminated
          // members of the lead group, but still reap the process and check
          // for the unreaped members afterwards.
          //
          if (!ie)
        ...
        else
        {
          ...
          // Note that this doesn't harm the process group leader since it has
          // already terminated. Also note that kill() should always return 0
          // here, since the leader is still present in the group (not being
          ...
          // Also note that such a behavior is actually permitted by POSIX.
          // Thus, let's just ignore the potential error.
          //
          //assert (r == 0 || (r == -1 && errno == EPERM));
        }

        // Note that we assume that since the process has already terminated
        // (or waitid() has failed) waitpid() will return immediately.
        //
        int es;
        r = waitpid (handle, &es, 0);
        handle = 0; // We have tried.
        ...
        else
        {
          if (WIFEXITED (es)) // Exited normally?
          {
            ...
            //   unrelated processes. OpenBSD and FreeBSD are at some risk
            //   here due to using randomized reuse strategies (fully
            //   randomized for OpenBSD and randomized increment for FreeBSD)
            //   in contrast to the sequential PID allocation on Linux, Mac
            //   OS, and NetBSD. As a result, for platforms that do fully
            //   randomized reuse, we disable this check.
            //
            ...
            // check here. Let's just keep the kill() calls as close to each
  ...
  try_wait ()
  {
    if (handle != 0)
    {
      ...
      else
      {
        ...
        if (r == 0)
        {
          if (info.si_pid == 0) // Not terminated yet.
            return nullopt;
  ...
  description () const
  {
    ...
    case SIGCHLD:   return "unreaped (grand)child process";
      ...
      : job (nullptr)
  {
    ...
    // If requested, create the job object.
    //
    auto_handle job;

    if (new_group)
    {
      // Note: the returned handle is not inheritable.
      //
      HANDLE j (CreateJobObject (0 /* lpJobAttributes */, 0 /* lpName */));
      ...
      // While we will be explicitly terminating the processes associated with
      // the job, by using TerminateJobObject(), let's also instruct the OS to
      // do the same when the job handle usage count drops to 0.
      //
      // Note that by using TerminateJobObject() we make sure that all the
      // child processes get terminated synchronously and no one, who may
      ...
      // actually aligns with POSIX, where grandchild processes may start/join
      // other groups.
      //
    ...
    auto_handle process;
    {
      ...
      for (size_t i (0);; ++i)
      {
        ...
        // Note: the returned pi.hProcess and pi.hThread handles are not
        //       inheritable.
        //
        if (!CreateProcess (
              ...
              job == nullhandle ? 0 : CREATE_SUSPENDED, // Creation flags.
        ...
        auto_handle thread (pi.hThread);

        if (job != nullhandle &&
            (!AssignProcessToJobObject (job.get (), process.get ()) ||
             ResumeThread (thread.get ()) == static_cast<DWORD> (-1)))
        {
          // Stash the error message, since it can be spoiled by
          // TerminateProcess(). Ignore a potential TerminateProcess() error.
          //
          string msg (last_error_msg ());
          TerminateProcess (process.get (), DBG_TERMINATE_PROCESS);
          fail (msg.c_str ());
        }
    ...
    assert (this->handle != nullptr && this->handle != INVALID_HANDLE_VALUE);
  ...
  wait (bool ie)
  {
    if (handle != nullptr)
    {
      ...
      handle = nullptr; // We have tried.

      if (job == nullptr) // Not a job leader?
      {
        if (e == NO_ERROR)
        {
          exit = process_exit (es, process_exit::as_status);
  ...
  kill ()
  {
    // Note that TerminateProcess() and TerminateJobObject() require an exit
    // code the process(es) will be terminated with. We could probably craft a
    // custom exit code that will be treated by the normal() function as an
    // abnormal termination. However, let's keep it simple and reuse the
    // existing (semantically close) error code.
    //
    if (handle != nullptr &&
        (job == nullptr
         ? !TerminateProcess   (handle, DBG_TERMINATE_PROCESS)
  ...
  description () const
  {
    ...
    // Presumably a job leader had unterminated processes in the job.
    //
    case STATUS_JOB_NOT_EMPTY: return "unterminated (grand)child process";
